...
//Elliot split
/*
void split(string message, string split_message[], int length, char split_character)
{
    ...
    {
        if (message[i] == split_character)
        {
            split_message[index] = message.substr(last_char, i - last_char);
            index++;
            last_char = i + 1;
        }
    }

    split_message[index] = message.substr(last_char, static_cast<int>(message.length()) - last_char);
}
*/
